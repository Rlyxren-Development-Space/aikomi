...
module.exports = async (client) => {
  try {
    ...
    /*readdirSync(`${process.cwd()}/events/RestEvents/`).forEach(async (file) => {
      ...
      client.rest.on(
        fileName,
        require(`../events/RestEvents/${file}`).bind(null, client)
      );
    ...
    */
    ...
    readdirSync(`${process.cwd()}/events/Guild/`).forEach(async (file) => {
      ...
      client.on(
        fileName,
        require(`../events/Guild/${file}`).bind(null, client)
      );
      eventCount = eventCount + 1;
    });
    ...
    readdirSync(`${process.cwd()}/events/Client/`).forEach(async (file) => {
      ...
      client.on(
        fileName,
        require(`../events/Client/${file}`).bind(null, client)
      );
      eventCount = eventCount + 1;
    });
    console.log(
      `[EVENT HANDLER]`.green,
      `Loaded `.white + `${eventCount} `.green + `Events`.white
    );
  } catch (e) {
    console.log(
      `[EVENT HANDLER]`.red,
      `Event Handler Error`.white,
      `\n\n${String(e.stack)}`
    );
  }
};
