  ...
  try {
    ...
    readdirSync(`${process.cwd()}/slashCommands/`).forEach((dir) => {
      if (lstatSync(`${process.cwd()}/slashCommands/${dir}`).isDirectory()) {
        ...
        if (cmdSetup) {
          readdirSync(`${process.cwd()}/slashCommands/${dir}/`).forEach(
            async (file) => {
              if (
                file.endsWith(".js") &&
                !lstatSync(
                  `${process.cwd()}/slashCommands/${dir}/${file}`
                ).isDirectory()
              ) {
                let pull = require(`${process.cwd()}/slashCommands/${dir}/${file}`);
                if (pull.name && pull.description) {
                  let Command = new SlashCommandBuilder()
                    .setName(`${cmdSetup.CmdName}`)
                    .setDescription(`${cmdSetup.CmdDescription}`);
                  ...
                  client.slashCommands.set(
                    String(cmdSetup.CmdName)
                      .replace(/\s+/g, "_")
                      .toLowerCase() + pull.name,
                    pull
                  );
                }
              }
        ...
        if (pull.name && pull.description) {
          let sub = new SlashCommandBuilder()
            .setName(String(pull.name).toLowerCase().replace(/\s+/g, "_"))
            .setDescription(String(pull.description));
          if (pull.options && pull.options.length > 0) {
            for (option of pull.options) {
              if (option.type && option.name && option.description) {
                let type = option.type;
                ...
                if (type == "String") {
                  sub.addStringOption((option) =>
                    option
                      .setName(String(name).toLowerCase())
                      .setDescription(String(description).toLowerCase())
                      .setRequired(required)
                  );
                }
                if (type == "User") {
        ...
        client.guilds.cache.forEach((guild) => {
          ...
          (async () => {
            try {
              ...
              console.log(
                `${`Successfully`.bold.green} Loaded ${`(/)`.brightGreen} ${
                  allCommands.length
                } Commands For: ${`${guild.name}`.brightRed}`
              );
            } catch (error) {
              console.log(
                `${`Unable`.bold.red} To Load ${`(/)`.brightGreen} ${
                  allCommands.length
                } Commands For: ${`${guild.name}`.brightRed}`
              );
            }
      ...
      if (config.loadSlashsGlobal == true) {
        ...
        try {
          ...
          console.log(
            `${`Successfully`.bold.green} Loaded ${`(/)`.brightGreen} ${
              allCommands.length
            } Commands For: ${`ALL POSSIBLE GUILDS`.brightRed}`
          );
        } catch (error) {
          console.log(
            `${`Unable`.bold.red} To Load ${`(/)`.brightGreen} ${
